...
async function api(path, options = {}) {
  const endpoints = path.startsWith("/api/") ? [path, `/cgi-bin/api.py/${path.slice(5)}`] : [path];
  let lastError;
...
document.querySelector("#joinProjectBtn").addEventListener("click", async () => {
  ...
  const state = await api("/api/member", {
    method: "POST",
    body: JSON.stringify({ name, className }),
  });
  currentMemberId = state.currentMemberId;
  localStorage.setItem("tripvote-member-id", currentMemberId);
  applySharedState(state);
  showScreen("workspace");
});
